...
function bufArray:offloadUdpChecksums(ipv4, l2Len, l3Len)
	...
	if ipv4 then
		...
		l3Len = l3Len or 40
		for i = 0, self.size - 1 do
			self.array[i].ol_flags = bit.bor(self.array[i].ol_flags, dpdk.PKT_TX_IPV6, dpdk.PKT_TX_UDP_CKSUM)
			self.array[i].tx_offload = l2Len + l3Len * 128
		end
...
function bufArray:offloadTcpChecksums(ipv4, l2Len, l3Len, n)
	...
	if ipv4 then
		...
		l3Len = l3Len or 40
		for i = 0, n - 1 do
			self.array[i].ol_flags = bit.bor(self.array[i].ol_flags, dpdk.PKT_TX_IPV6, dpdk.PKT_TX_TCP_CKSUM)
			self.array[i].tx_offload = l2Len + l3Len * 128
		end
...
function bufArray:setVlans(vlan, pcp, cfi)
	...
	for i = 0, self.size - 1 do
		...
		self.array[i].ol_flags = bit.bor(self.array[i].ol_flags, dpdk.PKT_TX_VLAN)
	end
end
