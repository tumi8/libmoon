...
void fdir_get_infos(int port_id);
...
//---------------------------Generic flow API-------------------------------------------
// start of copied and modified code from rte_flow.h (DPDK version 25.11)
...
enum rte_flow_conntrack_tcp_last_index {
	...
	RTE_FLOW_CONNTRACK_FLAG_SYN = 1u, /**< With SYN flag. */
	RTE_FLOW_CONNTRACK_FLAG_SYNACK = 2u , /**< With SYNACK flag. */
	RTE_FLOW_CONNTRACK_FLAG_FIN = 4u, /**< With FIN flag. */
	RTE_FLOW_CONNTRACK_FLAG_ACK = 8u, /**< With ACK flag. */
	RTE_FLOW_CONNTRACK_FLAG_RST = 16u, /**< With RST flag. */
};
...
rte_flow_dev_dump(uint16_t port_id, struct rte_flow *flow,
		int *file, struct rte_flow_error *error);
 ...
 */
bool
rte_flow_template_table_resizable(uint16_t port_id,
...
// end of copied and modified code from rte_flow.h (DPDK version 25.11)
]]
...
-- remove all filters from this device
...
-- add a new filter with the specified attributes, patterns and actions
-- the filter handle is returned
function createFilter(dev, flow_attr, filters, actions)
	-- error struct
	local flow_error = ffi.new("struct rte_flow_error")

	-- remove old filters from port
	local ok = C.rte_flow_flush(dev.id, flow_error)
	if ok ~= 0 then
		log:warn("removing of old filters failed. Exit code: " .. ok .. ". Root cause: " .. flowError[tonumber(flow_error.type)])
		if flow_error.message ~= nil then
			log:warn("Error message:")
	...
	for i=1,#filters do
		-- validate filter
		local ok = C.rte_flow_validate(dev.id, flow_attr, filters[i], actions, flow_error)
		if ok ~= 0 then
		...
		-- create filter
		local handle = C.rte_flow_create(dev.id, flow_attr, filters[i], actions, flow_error)
		if handle == nil then
			log:warn("Filter creation failed. Root cause: " .. flowError[tonumber(flow_error.type)])
			if flow_error.message ~= nil then
				log:warn("Error message:")
				log:warn("\t" .. ffi.string(flow_error.message))
			end
...
--- Generic filter, which can filter IPv4 UDP packets
--- the pattern is specifies using the filer structure with the following fields {src_addr,dst_addr}
--- fields, which are nil are masked out and ignored
--- @param filter structure, which specifies the pattern 
--- @param queue the index of the queue, where matching packets are directed
...
-- adds filters to match UDP PTP packets based on flex byte filters using 
-- the first two bytes of the UDP payload. The required pattern is dependent 
-- on the used NICs and is generated by functions implemented in the device 
-- driver lua files
